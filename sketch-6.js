...
var socialdistancingremoval = 0.0;                                   // Initial
var socialdistancing = false;                                             // Real-time
...
var daysRecoveryForNormalCases = 17;
...
function draw(){
  ...
  // text("Time: " + min(12.00, round(numberOfYears * 100)/100) + " months", width * (60/100), height - 47);
  // text("Dead: " + (round(min(percentagedied) *10000))/100 + "%", width * (3/8), height - 47);
}
...
function iterate(){
  ...
  for (var i = 0; i < col; i++){
    for (var j = 0; j < rows; j++){
      if (grid[i][j].personPresent() && grid[i][j].getPerson().getDaysInfected() >= 5){
        if (random(1) < (MORTRATE)) {
          grid[i][j].personDie();
          diedarray.push(grid[i][j]);
        }
      }
    }
  }
  */
  ...
  for (var i = 0; i < col; i++){
    for (var j = 0; j < rows; j++){
      if (grid[i][j].personPresent() && grid[i][j].getPerson().isInfected() && grid[i][j].getPerson().getDaysInfected() >= daysRecoveryForNormalCases * random(2)){
