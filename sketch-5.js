...
function setup(){
  ...
  population = 0;
  //while (population == 0 || countInfected()/population < 0.04) {
    grid = new Array(col);
    for (var i = 0; i < col; i++){
...
function draw(){
  ...
  text("Infected: " + (round((percentageinfected) *10000))/100 + "%", width * (1/10), height - 47);
  var onoff;
  if (socialdistancing) {
    onoff = "ON";
  } else {
  ...
  text("SD: " + onoff, width * (6/10), height - 47);

  if ((round((percentageinfected) *1000)/10) < 4) {
  ...
  this.movePerson = function(movable, newneigh, newgrid){
    if (socialdistancing && !this.getPerson().rulebreaker) {
      if (movable.length != 0){
        ...
        if (neighbourCount == 1 || neighbourCount == 2 || neighbourCount == 3){
